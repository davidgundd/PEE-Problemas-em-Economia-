...
ena_bacia <- list.files(path = "Dados/ENA BACIA/", pattern = "*.csv") %>% 
  ...
  summarise(ena = sum(ena, na.rm = T)) # Resumir por média da coluna "ena"
```
...
base_regressao <- base %>% 
  ...
  select(cnpj, data, mes_ano, dec, custo_acr, custo_acl, satisfacao, index_brl,
         tarifa_uso, tarifa_energia, ano, ena, uf, regiao)  %>% # selecionando variáveis de interesse 
  arrange(cnpj, data) %>% # ordenando dados
  filter(ano >= 2010 & ano <= 2020) %>% # selecionando período de interesse para análise
  mutate(d_mp = case_when(data >= '2012-09-01' ~ 1, T ~ 0), # criação da dummy para MP
         ...
         custo_acl = custo_acl/100,
         crise = case_when(regiao == 'SUDESTE' & ano >= 2013 & ano <= 2015 ~ 1,
                           T ~ 1)) %>%  # criação da dummy para o traifaço
...
base_regressao %>%  
  ggplot() + 
  geom_line(aes(data, ena, color = regiao), size = 1, ) + 
  geom_hline(yintercept = c(1000, 2000, 3000, 4000), linetype = "dashed", 
             color = "gray60", size = 0.25) +
  labs(title = 'ENA',
       subtitle = 'Dados referentes a todo o Brasil',
...
# Subset dos dados depois de 2012-09-11 até 2017-01

dados_pos <- subset(base_regressao, 
                       as.Date(data) >= as.Date("2012-09-11") & 
                       as.Date(data) <= as.Date("2017-01-01"))
...
base_regressao %>%
  ...
  geom_segment(aes(x = as.POSIXct("2012-09-11"), y = media[2], 
                  xend = as.POSIXct(max(as.POSIXct("2017-01-01"))), yend = media[2]),
...
base_regressao %>%
  ...
  geom_segment(aes(x = as.POSIXct("2012-09-11"), y = media[4], 
                  xend = as.POSIXct(max(as.POSIXct("2017-01-01"))), yend = media[4]),
              color = "red", size = 0.75, linetype = "longdash") +
  labs(title = 'Média mensal do Custo no Ambiente de Contratação Regulada',
       subtitle = 'Dados referentes a todo o Brasil',
       x = '',
...
base_regressao %>% 
  ...
  geom_segment(aes(x = as.POSIXct("2012-09-11"), y = media[10], 
                  xend = as.POSIXct(max(as.POSIXct("2017-01-01"))),
                  yend = media[10]),
              color = "red", size = 0.75, linetype = "longdash") +
  labs(title = 'Média mensal da Satisfação',
       subtitle = 'Dados referentes a todo o Brasil',
...
efeitos de índividuos ($\alpha_i$), efeitos de tempo ($\delta_t$), efeitos fixos de região ($\theta_r$) e uma
variável numérica para controlar a tendência ($\text{tendencia}$).\

$$satifacao_{irt} = \beta_1 + \beta_2custo\_acl_{rt} + \beta_3custo\_acl_{rt}\cdot d\_MP_t +\\\
\beta_3\Delta custo\_acr_{rt} + \beta_4\Delta tarifa\_energia_{irt} + \\
\beta_5DEC{irt}+\beta_6ENA_{ir}+\text{tendencia}+\alpha_i+\delta_t+\theta_r+\varepsilon_{irt}$$

```{r}
# Definição do modelo de regressão com efeitos fixos de cnpj e mês/ano
modelo_ef_tempo_diff <- plm(satisfacao ~ custo_acl + custo_acl*d_mp + diff_custo_acr + diff_custo_acr*d_mp +
                              diff_tarifa_energia + diff_tarifa_energia*d_mp + diff_tarifa_energia*custo_acl + 
                              ena + ena*custo_acl + ena*diff_custo_acr +
                              dec + dec*ena + 
                              regiao + tendencia, 
...
modelo_ef_tempo_diff2 <- plm(satisfacao ~ custo_acl*d_mp + diff_custo_acr + 
                         diff_tarifa_energia + diff_tarifa_uso + dec + ena + regiao + tendencia, 
                         data = base_regressao, # Base de dados utilizada
                         model = 'within', # Modelo de efeitos fixos
                         index = c('cnpj', 'mes_ano'), # Índices utilizados
                         effect = 'twoways' # Efeitos fixos de tempo
...
modelo_ef_tempo_diff2 <- plm(satisfacao ~ custo_acl*d_mp + diff_custo_acr*d_mp + 
                         diff_tarifa_energia*d_mp + dec + ena + regiao + tendencia, 
                         data = base_regressao, # Base de dados utilizada
                         model = 'within', # Modelo de efeitos fixos
                         index = c('cnpj', 'mes_ano'), # Índices utilizados
                         effect = 'twoways' # Efeitos fixos de tempo
...
```{r}
lmtest::bptest(satisfacao ~ custo_acl + custo_acl*d_mp + diff_custo_acr + diff_custo_acr*d_mp +
                         diff_tarifa_energia + diff_tarifa_energia*d_mp + diff_tarifa_energia*custo_acl + 
                           diff_tarifa_energia*diff_custo_acr + 
                           ena + ena*custo_acl + ena*diff_custo_acr + 
                           dec + dec*ena + 
                           regiao + tendencia + mes_ano + cnpj + regiao, 
               data = base_regressao, 
               studentize=F)
```
...
### Testes de correlação serial para as variáveis

```{r}
pbgtest(modelo_ef_tempo_diff)
```

```{r}
# Vetor com as variáveis de interesse
base_regressao_acf <- base_regressao %>% 
  select(cnpj, satisfacao, custo_acl, 
         diff_custo_acr, diff_tarifa_energia, dec, ena)

# selecione as variáveis que deseja fazer o gráfico
vars <- c("satisfacao", "custo_acl", "diff_custo_acr", "diff_tarifa_energia", "dec", "ena")

# faça um loop pelas variáveis
for (var in vars) {
  # faça um loop pelos CNPJs
  for (cnpj in unique(base_regressao_acf$cnpj)) {
    # filtre a base pelo CNPJ e pela variável
    print(paste0("Teste de Box-Ljung da variável ", var, " para ", cnpj))
    df <- base_regressao_acf %>% 
      filter(cnpj == !!cnpj) %>% 
      select(var)
    
    # faça o gráfico da FAC

    print(Box.test(ts(df[var]), lag = 24, type = "Ljung"))
  }
}
```
...
for (var in vars) {
  ...
  for (cnpj in unique(base_regressao_acf$cnpj)) {
    ...
    print(df %>% 
      ggAcf(var) +
      labs(title = paste0("FAC de ", var, " para ", cnpj)))
    
    # faça o gráfico da FACP

    print(df %>% 
      ggPacf(var) +
      labs(title = paste0("FACP de ", var, " para ", cnpj)))
  }
}
